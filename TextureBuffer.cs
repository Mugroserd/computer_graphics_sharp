...
namespace Renderer
{
    class TextureBuffer
    {
        ...
        public void ShadeBackgroundPixel(int x, int y, Point3F point1, Point3F point2, Point3F point3, float[,] pointsColors, ref int zBuf)
        {
            Color pixelValue;
        ...
        public void DrawTriangle(VertexParam coordinate1, VertexParam coordinate2, VertexParam coordinate3, VertexParam point1texture, VertexParam point2texture, VertexParam point3texture, ref int[,] zbuffer)
        {
            ...
            values = ConvertToTexture(vertices);
            ...
            else
            {
                for (int x = x0; x < x1; x++)
                    for (int y = y0; y < y1; y++)
                    {
                        ShadeBackgroundPixel(x, y, point1, point2, point3, values, ref zbuffer[x,y]);
                    }
            }
        }
    }
